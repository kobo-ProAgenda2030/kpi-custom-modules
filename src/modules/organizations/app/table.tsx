...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler = (property: keyof Profile) => (
    event: React.MouseEvent<unknown>
  ) => {
  ...
  return (
    <TableHead>
      <TableRow style={{ background: "#22B6C3", color: "white" }}>
        ...
        <TableCell key={"actions"} padding={"default"} align={"center"}>
          {""}
        </TableCell>
      </TableRow>
    </TableHead>
...
export function EnhancedTable({ profiles }: { profiles: Profile[] }) {
  ...
  return (
    <div>
      <Typography
        variant="h3"
        component="h2"
        style={{ color: "#2D2F3A", marginBottom: 10 }}
      >
        CAPyS
      </Typography>
      <Paper>
        <TableContainer>
          <Table aria-labelledby="tableTitle" aria-label="enhanced table">
            ...
            />
            <TableBody>
              {stableSort(profiles, getComparator(order, orderBy)).map(
                (row, index) => {
                  return (
